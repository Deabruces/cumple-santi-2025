...
const ROOMS = [
  {
    ...
    name: 'Los Mineros',
    emoji: '⛏️',
    description: 'Descubre los secretos de la mina abandonada y encuentra la salida',
    capacity: 6,
  },
  {
    id: 'bajo-presion',
    name: 'Bajo Presión',
    emoji: '🚢',
    description: 'Encuentra los tanques de oxígeno ocultos antes de que sea tarde',
    capacity: 6,
  },
];
...
export const Step5 = ({
  ...
  const [tempSelection, setTempSelection] = useState<string | null>(selectedRoom);
  ...
  useEffect(() => {
    const fetchRoomsStatus = async () => {
      try {
        ...
        } else if (status['bajo-presion'].count >= 6 && status.mineros.count < 6) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (tempSelection && selectedGuest && roomsStatus) {
      ...
      const roomCount = roomsStatus[tempSelection as keyof typeof roomsStatus].count;
  ...
  return (
    <form id='content-area' onSubmit={handleSubmit}>
          ...
          return (
            <label
              ...
              className={`block transition-all ${
                isFull ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:scale-102'
              ...
              >
                ...
                <div className='flex items-start gap-3 mb-3'>
                  ...
                  <div className='flex-1'>
                    <div className='flex items-center justify-between mb-1'>
                      <p className='text-green-400 font-bold pixel-text'>{room.name}</p>
                      {isFull && <span className='text-red-500 text-xs pixel-text'>LLENA</span>}
                    </div>
                    <p className='text-green-300 text-xs pixel-text mb-2'>{room.description}</p>
                    <div className='flex items-center gap-2 mb-2'>
                      ...
                      <p className={`text-xs pixel-text ${isFull ? 'text-red-500' : 'text-green-400'}`}>
                ...
                {roomData && roomData.guests.length > 0 && (
                  <div className='border-t border-green-800 pt-2'>
                    <p className='text-green-400 text-xs pixel-text mb-1'>Confirmados:</p>
